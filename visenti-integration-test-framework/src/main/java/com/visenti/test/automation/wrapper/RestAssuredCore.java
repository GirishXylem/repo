package com.visenti.test.automation.wrapper;
...
/**
 * Class Description: This is Rest API com.visenti.test.automation.wrapper
 * covers all Rest Call Methods built on top of Rest Assured
 */
public class RestAssuredCore {
	 ...
	 * @param basePath you can pass basepath alone or including basepath and
	 *                 relative path
	 */
	 ...
	 *
	 * @param baseURI            Initialize baseURI in RestAssuredCore constructor
	 * @param basePath           you can pass basepath alone or including basepath
	 *                           and relative path
	 * @param authenticationType Support as of now basic authentication type
	 * @param usernameOrCertPath Can pass either certification path or username,
	 *                           keep one variable so that can use two conditions
	 * @param password           supply password
	 */
	public RestAssuredCore(String baseURI, String basePath, String authenticationType, String usernameOrCertPath,
			String password) {
		RestAssured.baseURI = baseURI;
		RestAssured.basePath = basePath;
	...
	/**
	 *
	 * @throws IOException This is automatic logger which can be called before
	 *                     starting test
	 */
	public static void initializeLogger() throws IOException {
		printLogFile = new PrintStream(new BufferedOutputStream(
				new FileOutputStream(new File(returnProperties(CONFIG_FOLDER, "logConfigFile")), true)));
	 ...
	 * @return Instantiating Response class to hold response object after invoking
	 *         request
	 */
	 ...
	 *
	 * @param pathParamList Initializing dynamic value and use variable in rest
	 *                      endpoint for eg., test/{value} can add multiple values
	 */
	...
	public RestAssuredCore curlConfig() {
		request = request.config(new RestAssuredConfig()).redirects().follow(false);
		return this;
	}
}
